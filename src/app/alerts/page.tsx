...
export default function Alerts() {
    ...
    const visibleAlerts = alerts
        .filter(a => filter === 'All' || (filter === 'Resolved' && a.resolved) || (!a.resolved && a.severity.toLowerCase() === filter.toLowerCase()))
        ...
        <div className="flex h-full gap-6 flex-col">
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-2 scrollbar-thin scrollbar-thumb-navy-600 pb-10">
                {visibleAlerts.map(alert => {
                    const isRes = alert.resolved;
                    const bgClass = isRes ? 'opacity-50 grayscale hover:grayscale-0 border-l-emerald-500/30 bg-navy-900/40' : 'border-l-' + (alert.severity === 'high' ? 'red' : alert.severity === 'medium' ? 'yellow' : 'blue') + '-500 hover:-translate-y-0.5 shadow-md';
                    const colorClass = alert.severity === 'high' ? 'red' : alert.severity === 'medium' ? 'yellow' : 'blue';

                    return (
                        <div key={alert.id} className={'glass-card p-6 border-l-4 group transition-all duration-300 hover:shadow-lg ' + bgClass}>
                            <div className="flex justify-between items-start">
                                <div className="flex items-start space-x-4 max-w-3xl">
                                    {!isRes ? (
                                        <div className={'mt-1 h-3 w-3 rounded-full shrink-0 mt-1.5 shadow-lg ring-4 ring-navy-900 group-hover:animate-pulse bg-' + colorClass + '-500 shadow-' + colorClass + '-500/50'}></div>
                                    ) : (
                                        <CheckCircle className="w-5 h-5 text-emerald-500 shrink-0 mt-0.5" />
                                    )}
                                    <div>
                                        <div className="flex items-center space-x-3 mb-2">
                                            <h4 className={'font-bold text-lg ' + (isRes ? 'line-through text-navy-400' : 'text-white')}>{alert.title}</h4>
                                            <span className={'text-[10px] font-bold uppercase tracking-widest px-2.5 py-0.5 rounded border ' + (isRes ? 'border-navy-600 text-navy-500' : 'border-' + colorClass + '-500/30 text-' + colorClass + '-400 bg-' + colorClass + '-500/10')}>{alert.severity}</span>
                                        </div>
                                        <p className={'text-sm leading-relaxed ' + (isRes ? 'text-navy-500' : 'text-navy-200')}>{alert.description}</p>
